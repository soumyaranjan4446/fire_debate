...
class GraphBuilder:
    def __init__(self, device="cuda"):
        ...
        # We pass the encoder to prevent loading the model twice in VRAM
    ...
    def build_graph(self, log: DebateLog) -> HeteroData:
        """
        Builds a Homogeneous-style logic graph (Argument Nodes Only).
        This version focuses on Rhetoric and Logic Flow, removing noisy Evidence nodes.
        """
        data = HeteroData()
        ...
        # Pre-calculate Claim Embedding (Global Context / Goal Vector)
        # [1, 384]
        ...
        for t in log.turns:
            ...
            # We check how logical the argument is regarding the main claim
        ...
        # --- 2. CREATE NODE FEATURES (Feature Fusion) ---
        # Handle empty debate edge case
        if not arg_texts:
            # Fallback size: 384 (Arg) + 384 (Claim) + 3 (Scalars) = 771
            data['argument'].x = torch.zeros(1, 771, device=self.device) 
            data['argument'].num_nodes = 1
            return data

        # A. Encode Arguments [Batch, 384]
        arg_embeddings = self.encoder.encode(arg_texts, convert_to_tensor=True, device=self.device)
        
        # B. Expand Claim Context [Batch, 384]
        claim_context = claim_emb.repeat(len(arg_texts), 1)

        # C. Create Score Tensors [Batch, 1]
        ...
        # Node Vector = [Argument | Claim Context | Stance | Logic | Relevance]
        # Dimensions  = [384      + 384           + 1      + 1     + 1]       = 771
        features = torch.cat([arg_embeddings, claim_context, stances, l_scores, r_scores], dim=1)
        
        data['argument'].x = features
        data['argument'].num_nodes = features.size(0)

        # --- 3. BUILD EDGES (Sequential Flow) ---
        # We only model the flow of debate: Turn 0 -> Turn 1 -> Turn 2
        ...
        if num_turns > 1:
            ...
            # Empty edge index if only 1 turn
